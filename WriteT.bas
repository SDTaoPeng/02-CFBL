...
'
Public Sub WriteFile(t() As Double, xy() As Double, FileName As String)
    '声明本子程序使用的临时变量
    Dim Iw As Integer
    Dim Sw As String
    '打开文件供写入(若原有数据,会被清空)
    ...
    For Iw = LBound(t) To UBound(t)
        ...
        If xy(Iw, 1) >= 0 Then
           Sw = Sw & "+" & Format$(xy(Iw, 1), "0.0000000000") & " "
        Else
           Sw = Sw & Format$(xy(Iw, 1), "#0.0000000000") & " "
        End If
        If xy(Iw, 2) >= 0 Then
           Sw = Sw & "+" & Format$(xy(Iw, 2), "0.0000000000")
        Else
           Sw = Sw & Format$(xy(Iw, 2), "#0.0000000000")
        End If
        ...
        Print #1, Sw             '写入1行(Sw)
    Next Iw
        Sw = Format$(xymax1, "0.0000000000") & " "
        If Sumx1 >= 0 Then
           Sw = Sw & "+" & Format$(Sumx1, "0.0000000000") & " "
        Else
           Sw = Sw & Format$(Sumx1, "#0.0000000000") & " "
        End If
        If Sumy1 >= 0 Then
           Sw = Sw & "+" & Format$(Sumy1, "0.0000000000") & " "
        Else
           Sw = Sw & Format$(Sumy1, "#0.0000000000") & " "
        End If
    Print #1, Sw              '写入1行(Sw)
