...
'Fractional-order gradient descent learning of BP neural networks with Caputo derivative   FOBP
Dim Tpara As Double         '求指定函数的初始参数，如：1/(1-a)*gamma(1-a)
Dim Tgamma As Double        '求gammar函数
Dim Tsigmoid As Double      '求sigmoid函数
Dim cmin As Double             '求min(u,v)
Dim cv As Double             '求min(v)
Dim cw As Double             '求min(u)
Dim ctemp As Double         '辅助求c的中间变量，用于存储
Dim vc As Double            '存储(v-c)^(1-α)
Dim wc As Double            '存储(w-c)^(1-α)
Dim gfunction As Double     '设置g函数
Dim Tparameter As Double    '求全部系数乘积函数
...
Private Sub CmdStudySatrt_Click()    '单击"开始学习"按钮
 ...
 '---------------------------------------------------------------------------------------------------------
    ...
    For t = 1 To q: R(t) = SgnRnd(Rnd()) * 0.1 * Rnd(): Next t                     '输出层
...
'        ctemp = 1              '初始中间变量ctemp
 ...
 '---------------------------------------------添加CFBT的位置---------------------------------------------
            ...
            For j = 1 To p
                ...
                gfunction = 0
                For I = 1 To N
                    gfunction = gfunction + w(I, j) * s(k, j)         'g函数
                Next I
                
                For I = 1 To N
'                    Tsigmoid = SgnRnd(w(I, j))
                    wc = (w(I, j) - cmin) ^ (1 - 1)
                    Tparameter = Tpara * RMS * gfunction * wc * s(k, j)
'                    If Tparameter < 0 Then
'                        Tparameter = 1
'                    End If
                    'Tparameter = 1
                    w(I, j) = Tparameter * w(I, j)
                    s(k, j) = s(k, j) + w(I, j) * a(k, I)
                Next I
                s(k, j) = s(k, j) - O(j)     '阈值O(j)                        '(4.32)
                b(k, j) = fs(s(k, j))                                         '(4.33)
            Next j
            ...
            For t = 1 To q
                ...
                gfunction = 0
                For j = 1 To p
                    gfunction = gfunction + v(j, t) * s(k, t)         'g函数
                Next j
                
                For j = 1 To p
'                    Tsigmoid = SgnRnd(v(j, t))
                    vc = (v(j, t) - cmin) ^ (1 - 1)
                    Tparameter = Tpara * RMS * gfunction * vc
'                    If Tparameter < 0 Then
'                        Tparameter = 1
'                    End If
                    'Tparameter = 1
                    v(j, t) = Tparameter * v(j, t)
                    l(k, t) = l(k, t) + v(j, t) * b(k, j)
                Next j
                l(k, t) = l(k, t) - R(t)     '阈值r(j)
                c(k, t) = fs(l(k, t))
            Next t
...
'            For j = 1 To p                                                                   '隐含层
'                For t = 1 To q                                                               '输出层
'                    RMS = (y(k, t) - c(k, t)) * (y(k, t) - c(k, t)) / q                      '求MSE
'                    Tsigmoid = SgnRnd(v(j, t))
'                    Tparameter = Tpara * RMS * Tsigmoid
''                    If Tparameter < 0 Then
''                        Tparameter = 1
''                    End If
'                    Tparameter = 1
'                    v(j, t) = v(j, t) + Tparameter * RF * d(k, t) * b(k, j) '(4.38)
'                Next t
'            Next j
'            For t = 1 To q
...
'            For I = 1 To N                                                                  '输入层
'                For j = 1 To p                                                              '隐含层
'                    RMS = (y(k, I) - c(k, I)) * (y(k, I) - c(k, I)) / N                     '求MSE
'                    Tsigmoid = SgnRnd(w(I, j))
'                    Tparameter = Tpara * RMS * Tsigmoid
''                    If Tparameter < 0 Then
''                        Tparameter = 1
''                    End If
'                    Tparameter = 1
'                    w(I, j) = w(I, j) + Tparameter * BT * e(k, j) * a(k, I) '(4.40)
'                Next j
'            Next I
'            For j = 1 To p
...
'            Next j
            ...
            ctemp = v(j, 1)
                For t = 1 To q
                    ...
                    If v(j, t) < ctemp Then             '求最小ctemp
                        ctemp = v(j, t)
                    End If
                Next t
            If ctemp < cv Then                          '求最小cv
                cv = ctemp
            End If
            Next j
            For t = 1 To q
            ...
            ctemp = w(I, 1)
                For j = 1 To p
                    ...
                    If w(I, j) < ctemp Then             '求最小ctemp
                        ctemp = w(I, j)
                    End If
                Next j
            If ctemp < cw Then                          '求最小cw
                cw = ctemp
            End If
            Next I
            For j = 1 To p
...
'求gamma函数
'参考：https://bytes.com/topic/visual-basic/insights/662194-factorial-gamma-functions-vb
'参考：https://baike.baidu.com/item/%E4%BC%BD%E7%8E%9B%E5%87%BD%E6%95%B0/3540177?fromtitle=gamma%E5%87%BD%E6%95%B0&fromid=10819772&fr=aladdin
'当Dou1=0的时候，gamma=1        同时，当Dou1=1的时候，gamma=1
